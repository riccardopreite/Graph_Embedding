...
experiments = {
    ########## IMDB
    'imdb_review':  Experiment(
         'a', 'bert-base-cased', with_text=True, with_author_gender=False, with_manual=False, with_author_vec=False, is_imdb=True
     ),
    'imdbke_review':  Experiment(
         'a', 'bert-base-cased', with_text=True, with_author_gender=False, with_manual=False, with_author_vec=False, is_imdb=True, is_imdbke=True, mlp_dim=770
     ),
    ########## A
...
def run_on_val(name, cuda_device, extras_dir, df_train_path, df_val_path, output_dir, is_imdb, epochs=None, continue_training=False,
        batch_size=None):
    ...
    if not is_imdb:
        ...
        print("IMDB PREPARE DATA")
        train_dataloader, val_dataloader, vec_found_selector, val_df, val_y = experiment.imdb_prepare_data(df_train_path, df_val_path, extras_dir if name == "imdbke_review" else None)
...
def run_on_test(name, cuda_device, extras_dir, df_full_path, df_test_path, output_dir, is_imdb, epochs=None, continue_training=False,
        batch_size=None):
    ...
    if not is_imdb:
        ...
        train_dataloader, test_dataloader, vec_found_selector, test_df, _ = experiment.imdb_prepare_data(df_full_path, df_test_path, extras_dir if name == "imdbke_review" else None, test_set=True)
