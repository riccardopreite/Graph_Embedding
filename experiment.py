...
class Experiment(object):
    ...
    def __init__(self, task, bert_model, classifier_model=None, with_text=True, with_author_gender=True,
                 with_manual=True, with_author_vec=True, author_vec_switch=False, mlp_dim=None, is_imdb=False, is_imdbke=False):
        self.task = task
    ...
    def get_extra_cols(self):
        if self.with_manual:
            ...
            extra_cols = [None] * self.imdb_ke_cols
        else:
            extra_cols = []
        return extra_cols
    def film_extras(self, df, film2embedding):
        """
        Build matrix for extra data (i.e. author embeddings + gender)
        """
        FILM_DIM = len(next(iter(film2embedding.values())))
        extras = np.zeros((len(df), FILM_DIM))
    
        for i, review in enumerate(df['text']):
        # simple extras
            tt_id = df["tt_id"].values[i]
            if tt_id in film2embedding.keys():
                extra = film2embedding[ df["tt_id"].values[i] ] #one hot encoded without tt_id
                extras[i][:FILM_DIM] = extra
  
        return extras
    def imdb_prepare_data(self, df_train_path, df_val_path, relation_train_path=None, test_set=False):
        col = ['tt_id', 'text', 'is_positive', 'is_negative']
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        if relation_train_path is not None:

	    #relation_train_path = os.environ["RELATION_PATH"]
            with open(os.path.join(relation_train_path,"film2embedding.pickle"), 'rb') as f:
                print("loading relation...")
                relation_df = pickle.load(f)
                keys = list(relation_df.keys())
                key = keys[0]
                self.imdb_ke_cols = len(relation_df[key])
                print("relation laoded:",len(relation_df))
        else:
            relation_df = None
        if self.with_text:
        ...
        self.labels = ['is_positive', 'is_negative']
        if relation_df is not None:
             print("loading film extras...")
             train_extras = self.film_extras(train_df, relation_df)
             val_extras = self.film_extras(val_df, relation_df)
             print("extras loaded") 
        else:
             train_extras = None
             val_extras = None
        ...
        if self.with_text:
            train_texts = [t + '.\n' + train_df['text'].values[i] for i, t in enumerate(train_df['tt_id'].values)]
        else:
            train_texts = None
        ...
        if self.with_text:
            val_texts = [t + '.\n' + val_df['text'].values[i] for i, t in enumerate(val_df['tt_id'].values)]
        else:
            val_texts = None
        ...
        for epoch_num in range(self.epochs):
            ...
            for step_num, batch_data in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if self.with_text and (
                        self.with_manual or self.with_author_gender or self.with_author_vec) and not self.is_imdb:
                    # Full features
                    ...
                    #print("probas", probas)
                elif self.with_text and self.is_imdb:
                    # Text only
